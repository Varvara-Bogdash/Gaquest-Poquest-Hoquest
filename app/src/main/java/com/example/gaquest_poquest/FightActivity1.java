...
textViewFight.append("Вы применили усиление круцио. Осталось - " + (countOfKru - countClick) + " раз. ");
    ...
    private void dementorFight() {
        ...
        if (random == 0){
            ...
            textViewFight.append("Дементор учуял страх. Ваше здоровье " + (healthHero) + "/100  ");
            textViewDementor.setText("Дементор (сила - 20/100; усиления(каждое уменьшает здоровье на 10): чует страх - 5, забирает эмоции - " + ", превращается в человека - 5, оборонительное заклинание - 5; здоровье - " + healthDementor +"/100;");
        } else {
            if (random == 1){
                healthHero= healthHero - 10;
                textViewFight.append("Дементор забрал эмоции. Ваше здоровье " + (healthHero) + "/100  ");
            }else {
                healthHero= healthHero - 10;
                textViewFight.append("Дементор превратился в человека. Ваше здоровье  " + (healthHero) + "/100  ");
            }
        }
    }
}